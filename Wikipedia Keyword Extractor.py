...
while True:
    ...
    if event == sg.WIN_CLOSED or event == 'Cancel': # if user closes window or clicks cancel
        ...
        wikipedia_page = preprocess.wikipedia_web_scraper(link)
        cleaning_wiki_page = preprocess.data_preprocessing(wikipedia_page)
        keywords_noun = preprocess.noun_capture(cleaning_wiki_page)
        freq = visualise.frequency(keywords_noun)
        freq_top20 = dict((k, v) for k, v in freq.items() if v >= 10)
    
        #visualisations
        
        pie_chart = visualise.visualise_piechart(freq_top20)
        fig_canvas_agg = draw_figure(window['canvas'].TKCanvas, pie_chart)
        bc= visualise.bar_chart(freq_top20)
        fig_canvas_agg = draw_figure(window['canvas'].TKCanvas, bc)
        word_cloud= visualise.word_cloud_2(keywords_noun) 
